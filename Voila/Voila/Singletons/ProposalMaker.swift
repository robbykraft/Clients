...
//			UIScreen.navigationController.popToRootViewController()
	...
	func htmlProposal(project:Project, confirmKey:String)->String{
		...
		return pageHeader + self.titleForProject(project) + self.tableForProject(project) + self.tableOfCostsForProject(project) +  termsAndConditionsText + firebaseButton(key: confirmKey) + pageFooter
	}
